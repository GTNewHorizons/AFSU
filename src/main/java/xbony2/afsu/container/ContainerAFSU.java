...
public class ContainerAFSU extends ContainerFullInv<TileEntityAFSU> {
    ...
    public ContainerAFSU(EntityPlayer player, TileEntityAFSU tileentity) {
        ...
        for (int i = 0; i < 4; i++) addSlotToContainer(
                new SlotArmor(
                        player.inventory,
                        i,
                        ConfigHandler.AFSUArmxDispPosition,
                        ConfigHandler.AFSUArmyDispPosition + i * 18));

        addSlotToContainer(
                new SlotInvSlot(
                        tileentity.chargeSlot,
                        0,
                        ConfigHandler.AFSUCxDispPosition,
                        ConfigHandler.AFSUCyDispPosition));
        addSlotToContainer(
                new SlotInvSlot(
                        tileentity.dischargeSlot,
                        0,
                        ConfigHandler.AFSUDxDispPosition,
