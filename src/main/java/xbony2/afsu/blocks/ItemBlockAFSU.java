...
public class ItemBlockAFSU extends ItemBlock {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List tooltipList, boolean par4) {
        final String output = StatCollector.translateToLocal("ic2.item.tooltip.Output") + " "
                + TileEntityAFSU.MAX_OUTPUT
                + "EU/t";
        ...
        if (stack.hasTagCompound()) tooltipList.add(stored + stack.getTagCompound().getInteger("energy") + " EU");
