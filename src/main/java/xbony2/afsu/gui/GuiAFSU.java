...
public class GuiAFSU extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        this.fontRendererObj.drawString(
                ...
                ConfigHandler.AFSUyName,
                ConfigHandler.AFSUTextColor);

        this.fontRendererObj.drawString(
                this.level,
                ConfigHandler.AFSUxLevel,
                ConfigHandler.AFSUyLevel,
                ConfigHandler.AFSUTextColor);
        int energy = new Double(Math.min((this.container.base).energy, (container.base).maxStorage)).intValue();
        this.fontRendererObj.drawString(
                " " + energy,
                ConfigHandler.AFSUxEnergy,
                ConfigHandler.AFSUyEnergy,
                ConfigHandler.AFSUTextColor);
        this.fontRendererObj.drawString(
                "/" + (container.base).maxStorage,
                ConfigHandler.AFSUxEnergy2,
                ConfigHandler.AFSUyEnergy2,
                ConfigHandler.AFSUTextColor);

        String output = StatCollector.translateToLocalFormatted("ic2.EUStorage.gui.info.output", container.base.output);
        this.fontRendererObj.drawString(
                output,
                ConfigHandler.AFSUxEnergy3,
                ConfigHandler.AFSUyEnergy3,
                ConfigHandler.AFSUTextColor);

        GuiTooltipHelper.drawAreaTooltip(
                par1 - this.guiLeft,
                par2 - this.guiTop,
                (container.base).getredstoneMode(),
                ConfigHandler.AFSUToolTipminx,
                ConfigHandler.AFSUToolTipminy,
                ConfigHandler.AFSUToolTipmaxx,
                ConfigHandler.AFSUToolTipmaxy);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
        ...
        if (container.base.energy > 0.0D) {
            int i1 = new Double(((ConfigHandler.AFSUbarWidth * container.base.getChargeLevel()) / 1.6D) + 5.0D)
    ...
    protected void actionPerformed(GuiButton guibutton) {
        ...
        if (guibutton.id == ConfigHandler.AFSUGuiButtonid1)
            IC2.network.get().initiateClientTileEntityEvent(container.base, 0);
    }
}
